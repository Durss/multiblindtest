...
export default class Config {
  ...
  public static async loadPrivateCredentials(): Promise<void> {
    ...
    if (creds) {
      ...
      Logger.error(
        'MISSING FILE "credentials.conf" containing Twitch credentials',
      );
      Logger.error(
        "The file has been created, please fill the missing key values",
      );
      fs.writeFileSync(
        "credentials.conf",
        `SECRET;xxx
...
SMS_USER;xxx`,
  ...
  /**
   * Origins explicitly allowed to make cross-origin API calls.
   * Twitch extension/broadcaster origins and local dev origins are
   * handled separately (see HTTPServer.isAllowedOrigin).
   */
  public static get ALLOWED_ORIGINS(): string[] {
    return this.getEnvData({
      dev: [
        "https://multiblindtest.com",
        "https://www.multiblindtest.com",
        "https://multiblindtest.fr",
        "https://www.multiblindtest.fr",
      ],
      prod: [
        "https://multiblindtest.com",
        "https://www.multiblindtest.com",
  ...
  private static getEnvData(map: any): any {
    ...
    if (!this._ENV_NAME) {
      if (fs.existsSync(this._CONF_PATH)) {
        let content: string = fs.readFileSync(this._CONF_PATH, "utf8").trim();
        ...
        console.log("\n");
        console.log(
          'Creating env.conf file autmatically and set it to "standalone"\n\n',
        );
      }
    }
